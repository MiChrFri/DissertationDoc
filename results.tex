...
The standard deviation added and subtracted from the mean is the range that separates normal noise and peeks. 
...
In order to evaluate the efficiency of the code, we calculated the productivity by written source code lines per minutes
As both factors a the better the lower they are, they have been added to the score. 

\subsubsection{Complexity}
The code complexity is measured with two factors. The nesting depth within the code and the amount of lines of source code. In this case, all participants solved the same programming task and the lines of code actually detect how much value each line creates. The result is been divided by 6 which seemed to be a good weighting for the complexity of the given task.\\
...
The two factor of the code complexity also got added to the quality score. 

\subsubsection{Clean Code}
This factor is been calculated as several empty lines within the code files. Having single lines of white-space is alright to keep the structure of the code clean but several lines are just reducing the code that a developer can see on the screen. Also, a lower score is better and therefore added to the score as well.
In this section also the formatting is been looked at. The score gets two added points for code without indented code blocks and plus one point if it happened only a few times. Even when Python is based on intends, it is still a factor which makes some code unreadable. 
